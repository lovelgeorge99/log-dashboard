    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <footer className="border-t border-border bg-card px-6 py-4">
        <div className="flex flex-col gap-2 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
          <p>
            API Endpoints: <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">GET /api/logs</code>{' '}
            <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">POST /api/logs</code>{' '}
            <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">GET /api/logs/stats</code>
          </p>
          <p>Polling every {POLL_INTERVAL / 1000}s {isLive ? '(active)' : '(paused)'}</p>
        </div>
      </footer>
    </div>
  );
